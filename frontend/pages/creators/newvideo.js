...
function ContractsPage() {
    ...
    const handleUpload = async (creatorCollectionAddress) => { //creatorCollectionAddress = creator profle smart contract
        if(!selectedFile){
          setErrorMsg({
      ...
      //uploading state as true
        ...
        const options1 = {
            ...
            headers: {
              'x-tva-sa-id': 'srvacc_ac6dyjtc3fwc2ecmtf1yzc099',
              'x-tva-sa-secret': 't30j9d9kkd7nfa9fr4ybikzw4gnjtpd6',
            },
          };
        ...
        const options3 = {
            ...
            //   nft_collection: creatorCollectionAddress,
              drm_rules:[{nft_collection:creatorCollectionAddress,chain_id:365}],//enable wich nft have acces to it//365 testnet , 361 mainnet
    ...
    return (
        <div>
            ...
            <button onClick={()=>{handleUpload(collectionAddress)}}>Upload</button>
            <h2>created video:</h2>
            {video}
             {/*show messages errors, add notification for it*/}
            {errorMsg.msg}
            {/* when uploading show the video progress */}
            {uploading ? <div>this can take few moments please dont leavt his page,
            video progress: {uploadProgress}</div> : <></>}
          </div>
